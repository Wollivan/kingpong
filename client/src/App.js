...
import { PLAYERS_API, GAMES_API, CHALLENGES_API, TOURNAMENTS_API } from "./utils/api";
...
function App() {
  ...
  const getPlayersList = (tournamentCode) => {
    axios
      ...
      .catch((err) => console.log("Something went wrong while fetching the player list data: ", err));
  };
  ...
  return (
    <BrowserRouter>
      <>
        {tournamentCode && <Nav />}
        {/* {!localStorage.getItem("seenNotice") && <NoticeModal />} */}
        <div className="App">
          {/* <LockScreen /> */}
          {/* <PageHeader /> */}
          <Switch>
            <Route
              ...
              render={(routerProps) => {
                return (
                  <Home setTournamentCode={setTournamentCode} tournamentCode={tournamentCode} tournaments={tournaments} />
                );
              }}
            />
            <Route
            ...
            />
            <Route path="/chat" exact component={Chat} tournamentCode={tournamentCode} />
          </Switch>
        </div>
      </>
    </BrowserRouter>
